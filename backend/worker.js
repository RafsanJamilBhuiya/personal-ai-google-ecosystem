...
function originFor(env, request) {
  ...
  if (!configured) return env.APP_ENV === "production" ? "null" : "*";
...
function headers(origin) {
  return { "access-control-allow-origin": origin, "access-control-allow-methods": "GET,POST,OPTIONS", "access-control-allow-headers": "content-type,authorization,x-request-id", "access-control-max-age": "86400", "x-content-type-options": "nosniff", "referrer-policy": "no-referrer", "x-frame-options": "DENY" };
}
...
export default { async fetch(request, env) {
  ...
  const toolRegistry = createToolRegistry(env);
  const router = createRouter(new Map([
    ...
    ["POST /api/command", async (req, context) => { let body; try { body = await req.json(); } catch { throw new HttpError(400, "INVALID_JSON", "Request body must be valid JSON"); } body = validateCommandPayload(body); const parsed = parseCommand(body); const planned = planTask(parsed); const commands = createCommandRegistry({ env, toolRegistry }); const handler = commands.get(body.intent || parsed.intent) || commands.get("task.create"); if (!handler) throw new HttpError(400, "UNKNOWN_COMMAND", "Unsupported command"); const result = await handler({ ...body, requestId: context.requestId, parsed, plan: planned, input: body.input || body.metadata || {} }); await appendActivityLog(env, { source: body.source || "api", action: parsed.intent, task_id: result?.task_id || result?.result?.task_id || "", service: parsed.intent?.startsWith("google.") ? parsed.intent.split(".")[1] : "system", status: "completed", message: "Command executed" }).catch(() => {}); return json({ ok: true, requestId: context.requestId, intent: parsed.intent, plan: planned, result }, 200, origin); }]
  ]));
