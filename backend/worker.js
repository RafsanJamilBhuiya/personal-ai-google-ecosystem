...
function headers(origin) { return {"access-control-allow-origin":origin,"access-control-allow-methods":"GET,POST,OPTIONS","access-control-allow-headers":"content-type,authorization,x-request-id","access-control-max-age":"86400","x-content-type-options":"nosniff","referrer-policy":"no-referrer","x-frame-options":"DENY"}; }
const json=(data,status,origin)=>{const response=jsonResponse(data,status,origin);Object.entries(headers(origin)).forEach(([k,v])=>response.headers.set(k,v));return response;};
...
export default { async fetch(request, env) {
  ...
  const providers=createProviderRegistry(env), aiRouter=new AIRouter({providers,env}), googleServices=createGoogleServiceRegistry(env), toolRegistry=createToolRegistry(env), router=createRouter(new Map([
    ...
    ["GET /api/realtime",async req=>{const id=new URL(req.url).searchParams.get("taskId");if(!id)throw new HttpError(400,"TASK_ID_REQUIRED","taskId is required");const response=taskEventStream(env,id);Object.entries(headers(origin)).forEach(([k,v])=>response.headers.set(k,v));return response;}],
    ...
    ["POST /api/tasks",async req=>{const task=await createTask(env,await req.json());return json({ok:true,task},201,origin)}],
