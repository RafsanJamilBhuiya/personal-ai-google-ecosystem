...
export default {async fetch(request,env){const origin=originFor(env);if(request.method==="OPTIONS")return new Response(null,{status:204,headers:headers(origin)});const providers=createProviderRegistry(env);const aiRouter=new AIRouter({providers,env});const googleServices=createGoogleServiceRegistry(env);const toolRegistry=createToolRegistry(env);const router=createRouter(new Map([
