...
export default {async fetch(request,env){
 ...
 const router=createRouter(new Map([
  ...
  ["GET /api/auth/google/callback",async req=>{const u=new URL(req.url),result=await completeAdminGoogleLogin(env,u.searchParams.get("code"),u.searchParams.get("state"));return new Response(null,{status:302,headers:{location:`${env.FRONTEND_ORIGIN}/pages/login.html?challenge=${encodeURIComponent(result.challenge)}`}});}],
